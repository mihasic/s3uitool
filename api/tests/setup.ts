...
// The dev machine's ~/.aws would otherwise make the profile list non-deterministic.
process.env.ENABLE_PROFILE_DISCOVERY ||= "0";

// A second profile pointing at the same emulators, so profile-scoped routing is exercised.
process.env.PROFILE_alt_LABEL ||= "Alternate";
process.env.PROFILE_alt_S3_ENDPOINT_URL ||= process.env.AWS_S3_ENDPOINT_URL;
process.env.PROFILE_alt_SQS_ENDPOINT_URL ||= process.env.AWS_SQS_ENDPOINT_URL;
process.env.PROFILE_alt_REGION ||= process.env.AWS_DEFAULT_REGION;
process.env.PROFILE_alt_ACCESS_KEY_ID ||= process.env.AWS_ACCESS_KEY_ID;
process.env.PROFILE_alt_SECRET_ACCESS_KEY ||= process.env.AWS_SECRET_ACCESS_KEY;
